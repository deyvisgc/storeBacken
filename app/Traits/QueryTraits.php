...
trait QueryTraits
{
    ...
    function obtenerCliente($idPersona, $desde, $hasta, $numero, $tipoDocumento, $departamento, $provincia, $distrito) {
        ...
        if ($numero) {
            $query->where('per.per_numero_documento', $numero);
        }
        if ($tipoDocumento) {
        ...
        if ($provincia) {
            $query->where('per.id_provincia', $provincia);
        }
        if ($distrito) {
            $query->where('per.id_distrito', $distrito);
        }
        $query->leftJoin('ubigeo_peru_departments as dep', 'per.id_departamento', '=', 'dep.id')
